...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        const dbSnapshots = await database.all(`SELECT 
       ...
       ORDER BY date DESC`, [userId]);
        ...
        const snapshots = await Promise.all(dbSnapshots.map(async (snapshot) => {
            const accounts = await database.all('SELECT * FROM accounts WHERE snapshot_id = ?', [snapshot.id]);
...
router.get('/:id', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const dbSnapshot = await database.get(`SELECT 
        id, user_id, date, total_net_worth, total_assets, total_liabilities,
        monthly_gain, dollars_per_hour, portfolio_change, hours_in_month,
        created_at, updated_at
       FROM snapshots 
       WHERE id = ? AND user_id = ?`, [id, userId]);
        ...
        const accounts = await database.all('SELECT * FROM accounts WHERE snapshot_id = ?', [id]);
...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        for (const account of accounts) {
            ...
            await database.run(`INSERT INTO accounts (
        ...
        const createdDbSnapshot = await database.get(`SELECT 
        id, user_id, date, total_net_worth, total_assets, total_liabilities,
        monthly_gain, dollars_per_hour, portfolio_change, hours_in_month,
        created_at, updated_at
       FROM snapshots 
       WHERE id = ?`, [snapshotId]);
        const createdAccounts = await database.all('SELECT * FROM accounts WHERE snapshot_id = ?', [snapshotId]);
...
router.delete('/:id', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const snapshot = await database.get('SELECT id FROM snapshots WHERE id = ? AND user_id = ?', [id, userId]);
        if (!snapshot) {
            return res.status(404).json({ error: 'Snapshot not found' });
        }
